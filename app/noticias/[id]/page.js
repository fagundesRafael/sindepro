...
export default function NoticiaDetalhada() {
    ...
    const verificarCurtida = async () => {
        ...
        try {
            // Requisição GET para verificar a curtida (assumindo que /api/curtidas/check/[id] existe e usa GET)
            const response = await fetch(`/api/curtidas/check/${id}?userId=${session.user.id}`);
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                // Verifica se o erro é 404 (Not Found) que pode indicar que a rota GET não existe
                if (response.status === 404) {
                     console.warn(`Aviso: Endpoint GET /api/curtidas/check/${id} não encontrado.`);
                     // Decide o que fazer: pode assumir que não curtiu ou mostrar um erro diferente.
                     // Vamos assumir que não curtiu por enquanto para não travar.
                     setJaCurtiu(false);
            ...
            // Não mostra notificação para erro silencioso de verificação, mas loga o erro.
        }
    };
    ...
    const handleCurtir = async () => {
        ...
        try {
            // Chamando a API POST /api/curtidas/[id] para curtir/descurtir
            const response = await fetch(`/api/curtidas/${id}`, { // Chamando a API com ID na URL
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                // O body pode ser vazio se a API pegar o userId da sessão no backend
            });

            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                // Verifica se é erro de conta inativa vindo da API
    ...
    return (
        <div className="min-h-screen flex flex-col bg-white">
            ...
            {/* Conteúdo Principal */}
            <main className="flex-grow mt-2 rounded-lg bg-white mx-[10%] p-4 sm:p-6 lg:p-8">
                {/* Mensagem de Feedback Geral (Topo) */}
                {mensagemGeral.texto && (
                    <div className={`fixed top-20 right-4 p-4 rounded-lg shadow-lg z-50 ${mensagemGeral.tipo === 'sucesso' ? 'bg-green-500' : 'bg-red-500'
                ...
                <article className="bg-white rounded-lg shadow-lg overflow-hidden mb-12">
                    ...
                    {/* Coluna da Esquerda: Imagem (FLUTUADA) */}
                    <div className="w-full md:w-1/2 md:float-left md:mr-6 lg:md:mr-8 mb-4 md:mb-0">
                    ...
                    {/* Coluna da Direita: Detalhes e Conteúdo (FLUI AO REDOR E ABAIXO) */}
                    <div className="p-4 sm:p-6 lg:p-8"> {/* Padding aplicado aqui */}
                        ...
                        <div className="prose prose-sm sm:prose-base max-w-none text-gray-800 leading-relaxed">
                            {noticia.descricao?.split('\n').map((paragrafo, index) => (
                                paragrafo.trim() ? <p key={index}>{paragrafo}</p> : null
                            ))}
                        </div>
                    </div>
                    ...
                    {/* Seção de Curtidas */}
                    <div className="flex items-center gap-4 p-4 sm:p-6 lg:p-8 border-t">
                        <h2 className="text-lg font-semibold text-gray-800 mr-4">Gostou?</h2>
                        <button
                            onClick={handleCurtir}
                            className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-200 ease-in-out text-sm ${jaCurtiu
                        ...
                        >
                            <svg className={`w-5 h-5 transition-colors ${jaCurtiu ? 'text-white' : 'text-red-500'}`}
                            ...
                            session.user?.isActive ? (
                                <form onSubmit={handleComentario} className="mb-8">
                                    ...
                                    <button
                                        ...
                                        disabled={!comentario.trim() || status !== 'authenticated'}
                                    >
                                        Enviar Comentário
                                    </button>
                                </form>
